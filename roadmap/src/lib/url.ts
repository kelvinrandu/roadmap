...
export const getTwitterShareUrl = ({
  ...
  return `https://twitter.com/intent/tweet?text=${text}&url=${encodeURI(
    prefixHost(url)
  )}`;
};
...
export const getFacebookShareUrl = ({
  ...
  return `https://www.facebook.com/sharer/sharer.php?quote=${text}&u=${encodeURI(
    prefixHost(url)
  )}`;
};

export const getRedditShareUrl = ({
  text,
  url,
}: {
  text: string;
  ...
  return `https://www.reddit.com/submit?title=${text}&url=${encodeURI(
    prefixHost(url)
  )}`;
};

export const getHnShareUrl = ({ text, url }: { text: string; url: string }) => {
  return `https://news.ycombinator.com/submitlink?t=${text}&u=${prefixHost(
    url
  )}`;
};
